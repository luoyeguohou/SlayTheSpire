 ...
 * @since 3.0.0
 */
export const union = <A>(E: Eq<A>): Se.Semigroup<ReadonlyNonEmptyArray<A>>['concat'] => {
  const uniqE = uniq(E)
...
/**
 * Places an element in between members of a `ReadonlyNonEmptyArray`, then folds the results using the provided `Se.Semigroup`.
 *
 * @example
 * import * as S from 'fp-ts/string'
 * import { intercalate } from 'fp-ts/ReadonlyNonEmptyArray'
 *
 * assert.deepStrictEqual(intercalate(S.Se.Semigroup)('-')(['a', 'b', 'c']), 'a-b-c')
 *
 * @since 3.0.0
 */
export const intercalate = <A>(S: Se.Semigroup<A>): ((middle: A) => (as: ReadonlyNonEmptyArray<A>) => A) => {
...
/**
 * Identifies an associative operation on a type constructor. It is similar to `Se.Semigroup`, except that it applies to
 * types of kind `* -> *`.
 *
...
/**
 * **Note**. The constraint is relaxed: a `Se.Semigroup` instead of a `Monoid`.
 *
 * @category Foldable
 * @since 3.0.0
 */
export const foldMap = <S>(S: Se.Semigroup<S>) => <A>(f: (a: A) => S) => (fa: ReadonlyNonEmptyArray<A>): S =>
 ...
 * @since 3.0.0
 */
export const getUnionSemigroup = <A>(E: Eq<A>): Se.Semigroup<ReadonlyNonEmptyArray<A>> => ({
  concat: union(E)
})
 ...
 * @since 3.0.0
 */
export const concatAll = <A>(S: Se.Semigroup<A>) => (fa: ReadonlyNonEmptyArray<A>): A =>
