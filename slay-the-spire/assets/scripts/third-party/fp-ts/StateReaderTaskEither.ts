...
export interface StateReaderTaskEither<S, R, E, A> {
  (s: S): RTE.ReaderTaskEither<R, E, readonly [A, S]>
}
 ...
 * @since 3.0.0
 */
export const rightReader = <R, A, S, E = never>(ma: R.Reader<R, A>): StateReaderTaskEither<S, R, E, A> =>
 ...
 * @since 3.0.0
 */
export const leftReader = <R, E, S, A = never>(me: R.Reader<R, E>): StateReaderTaskEither<S, R, E, A> =>
 ...
 * @since 3.0.0
 */
export const chainReaderTaskEitherKW = <A, R, E2, B>(f: (a: A) => RTE.ReaderTaskEither<R, E2, B>) => <S, E1>(
  ma: StateReaderTaskEither<S, R, E1, A>
 ...
 * @since 3.0.0
 */
export const evaluate: <S>(s: S) => <R, E, A>(ma: StateReaderTaskEither<S, R, E, A>) => RTE.ReaderTaskEither<R, E, A> =
  /*#__PURE__*/
 ...
 * @since 3.0.0
 */
export const execute: <S>(s: S) => <R, E, A>(ma: StateReaderTaskEither<S, R, E, A>) => RTE.ReaderTaskEither<R, E, S> =
  /*#__PURE__*/
...
export const traverseReadonlyNonEmptyArrayWithIndex = <A, S, R, E, B>(
  ...
  _.tail(as).reduce<Promise<E.Either<E, [NonEmptyArray<B>, S]>>>(
    (acc, a, i) =>
      acc.then((ebs) =>
        _.isLeft(ebs)
          ? acc
