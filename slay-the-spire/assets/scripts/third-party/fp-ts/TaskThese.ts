 ...
 * @since 3.0.0
 */
export interface TaskThese<E, A> extends T.Task<TH.These<E, A>> { }
 ...
 * @since 3.0.0
 */
export const rightTask: <A, E = never>(ma: T.Task<A>) => TaskThese<E, A> =
  /*#__PURE__*/
  TT.rightF(T.Functor)
 ...
 * @since 3.0.0
 */
export const leftTask: <E, A = never>(me: T.Task<E>) => TaskThese<E, A> =
  /*#__PURE__*/
  TT.leftF(T.Functor)
...
export const traverseReadonlyNonEmptyArrayWithIndexSeq = <E>(S: Semigroup<E>) => <A, B>(
  ...
  _.tail(as).reduce<Promise<TH.These<E, NonEmptyArray<B>>>>(
    (acc, a, i) =>
      acc.then((ebs) =>
        TH.isLeft(ebs)
          ? acc
