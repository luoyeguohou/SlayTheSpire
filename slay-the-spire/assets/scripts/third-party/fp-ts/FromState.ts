...
export interface FromState<F> {
  readonly URI?: F
  readonly fromState: <S, A>(fa: S.State<S, A>) => HKT2<F, S, A>
}
...
export function fromStateK<F extends URIS4>(
  F: FromState4<F>
): <A extends ReadonlyArray<unknown>, S, B>(f: (...a: A) => S.State<S, B>) => <R, E>(...a: A) => Kind4<F, S, R, E, B>
export function fromStateK<F extends URIS3>(
  F: FromState3<F>
): <A extends ReadonlyArray<unknown>, S, B>(f: (...a: A) => S.State<S, B>) => <E>(...a: A) => Kind3<F, S, E, B>
export function fromStateK<F extends URIS3, E>(
  F: FromState3C<F, E>
): <A extends ReadonlyArray<unknown>, S, B>(f: (...a: A) => S.State<S, B>) => (...a: A) => Kind3<F, S, E, B>
export function fromStateK<F extends URIS2>(
  F: FromState2<F>
): <A extends ReadonlyArray<unknown>, S, B>(f: (...a: A) => S.State<S, B>) => (...a: A) => Kind2<F, S, B>
export function fromStateK<F>(
  F: FromState<F>
): <A extends ReadonlyArray<unknown>, S, B>(f: (...a: A) => S.State<S, B>) => (...a: A) => HKT2<F, S, B>
export function fromStateK<F>(
  F: FromState<F>
): <A extends ReadonlyArray<unknown>, S, B>(f: (...a: A) => S.State<S, B>) => (...a: A) => HKT2<F, S, B> {
...
export function chainStateK<M extends URIS4>(
  F: FromState4<M>,
  M: Chain4<M>
): <A, S, B>(f: (a: A) => S.State<S, B>) => <R, E>(ma: Kind4<M, S, R, E, A>) => Kind4<M, S, R, E, B>
export function chainStateK<M extends URIS3>(
  F: FromState3<M>,
  M: Chain3<M>
): <A, S, B>(f: (a: A) => S.State<S, B>) => <E>(ma: Kind3<M, S, E, A>) => Kind3<M, S, E, B>
export function chainStateK<M extends URIS2>(
  F: FromState2<M>,
  M: Chain2<M>
): <A, S, B>(f: (a: A) => S.State<S, B>) => (ma: Kind2<M, S, A>) => Kind2<M, S, B>
export function chainStateK<M>(
  F: FromState<M>,
  M: Chain<M>
): <A, S, B>(f: (a: A) => S.State<S, B>) => (ma: HKT2<M, S, A>) => HKT2<M, S, B>
export function chainStateK<M extends URIS2>(
  F: FromState2<M>,
  M: Chain2<M>
): <A, S, B>(f: (a: A) => S.State<S, B>) => (ma: Kind2<M, S, A>) => Kind2<M, S, B> {
