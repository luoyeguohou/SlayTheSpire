 ...
 * @since 3.0.0
 */
export const matchW = <B, A, C>(onEmpty: Lazy<B>, onNonEmpty: (as: RNEA.ReadonlyNonEmptyArray<A>) => C) => (
  as: ReadonlyArray<A>
 ...
 * @since 3.0.0
 */
export const scanLeft = <B, A>(b: B, f: (b: B, a: A) => B) => (as: ReadonlyArray<A>): RNEA.ReadonlyNonEmptyArray<B> => {
  const len = as.length
 ...
 * @since 3.0.0
 */
export const scanRight = <B, A>(b: B, f: (a: A, b: B) => B) => (as: ReadonlyArray<A>): RNEA.ReadonlyNonEmptyArray<B> => {
  const len = as.length
  const out = new Array(len + 1) as [B, ...Array<B>]
  out[len] = b
 ...
 * @since 3.0.0
 */
export const isNonEmpty: <A>(as: ReadonlyArray<A>) => as is RNEA.ReadonlyNonEmptyArray<A> = _.isNonEmpty
 ...
 * @since 3.0.0
 */
export const prependW: <B>(head: B) => <A>(tail: ReadonlyArray<A>) => RNEA.ReadonlyNonEmptyArray<A | B> = RNEA.prependW

/**
 * Prepend an element to the front of a `ReadonlyArray`, creating a new `RNEA.ReadonlyNonEmptyArray`.
 *
 * @example
 ...
 * @since 3.0.0
 */
export const prepend: <A>(head: A) => (tail: ReadonlyArray<A>) => RNEA.ReadonlyNonEmptyArray<A> = RNEA.prepend
 ...
 * @since 3.0.0
 */
export const appendW: <B>(end: B) => <A>(init: ReadonlyArray<A>) => RNEA.ReadonlyNonEmptyArray<A | B> = RNEA.appendW

/**
 * Append an element to the end of a `ReadonlyArray`, creating a new `RNEA.ReadonlyNonEmptyArray`.
 *
 * @example
 ...
 * @since 3.0.0
 */
export const append: <A>(end: A) => (init: ReadonlyArray<A>) => RNEA.ReadonlyNonEmptyArray<A> = RNEA.append
 ...
 * @since 3.0.0
 */
export const insertAt = <A>(i: number, a: A) => (as: ReadonlyArray<A>): Option<RNEA.ReadonlyNonEmptyArray<A>> => {
  if (i < 0 || i > as.length) {
    return _.none
  }
  if (isNonEmpty(as)) {
...
export const chop = <A, B>(
  f: (as: RNEA.ReadonlyNonEmptyArray<A>) => readonly [B, ReadonlyArray<A>]
 ...
 * @since 3.0.0
 */
export const chunksOf = (n: number): (<A>(as: ReadonlyArray<A>) => ReadonlyArray<RNEA.ReadonlyNonEmptyArray<A>>) => {
 ...
 * @since 3.0.0
 */
export const some = <A>(predicate: Predicate<A>) => (as: ReadonlyArray<A>): as is RNEA.ReadonlyNonEmptyArray<A> =>
  as.some(predicate)
