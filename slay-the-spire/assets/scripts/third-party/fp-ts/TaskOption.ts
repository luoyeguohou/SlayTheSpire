 ...
 * @since 3.0.0
 */
export const match: <B, A>(onNone: () => B, onSome: (a: A) => B) => (ma: TaskOption<A>) => T.Task<B> =
  /*#__PURE__*/
  OT.match(T.Functor)
...
export const matchW: <B, A, C>(
  onNone: () => B,
  onSome: (a: A) => C
) => (ma: TaskOption<A>) => T.Task<B | C> = match as any

/**
 * @category destructors
 * @since 3.0.0
 */
export const matchE: <B, A>(onNone: () => T.Task<B>, onSome: (a: A) => T.Task<B>) => (ma: TaskOption<A>) => T.Task<B> =
  /*#__PURE__*/
  OT.matchE(T.Monad)
...
export const matchEW: <B, A, C>(
  onNone: () => T.Task<B>,
  onSome: (a: A) => T.Task<C>
) => (ma: TaskOption<A>) => T.Task<B | C> = matchE as any

/**
 * @category destructors
 * @since 3.0.0
 */
export const getOrElse: <A>(onNone: Lazy<A>) => (fa: TaskOption<A>) => T.Task<A> =
  /*#__PURE__*/
 ...
 * @since 3.0.0
 */
export const getOrElseW: <B>(onNone: Lazy<B>) => <A>(ma: TaskOption<A>) => T.Task<A | B> = getOrElse as any

/**
 * @category destructors
 * @since 3.0.0
 */
export const getOrElseE: <A>(onNone: Lazy<T.Task<A>>) => (fa: TaskOption<A>) => T.Task<A> =
  /*#__PURE__*/
 ...
 * @since 3.0.0
 */
export const getOrElseEW: <B>(onNone: Lazy<T.Task<B>>) => <A>(ma: TaskOption<A>) => T.Task<A | B> = getOrElseE as any
...
/**
 * Transforms a `Promise` that may reject to a `Promise` that never rejects and returns an `O.Option` instead.
 *
 ...
 * @since 3.0.0
 */
export const chainOptionK: <A, B>(f: (a: A) => O.Option<B>) => (ma: TaskOption<A>) => TaskOption<B> =
  /*#__PURE__*/
...
export const traverseReadonlyNonEmptyArrayWithIndexSeq = <A, B>(f: (index: number, a: A) => TaskOption<B>) => (
  ...
  _.tail(as).reduce<Promise<O.Option<NonEmptyArray<B>>>>(
    (acc, a, i) =>
      acc.then((obs) =>
        _.isNone(obs)
          ? acc
