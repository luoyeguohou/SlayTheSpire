 ...
 * @since 3.0.0
 */
export const insertAt = <K>(E: Eq<K>): (<A>(k: K, a: A) => (m: ReadonlyMap<K, A>) => O.Option<ReadonlyMap<K, A>>) => {
 ...
 * @since 3.0.0
 */
export const updateAt = <K>(E: Eq<K>): (<A>(k: K, a: A) => (m: ReadonlyMap<K, A>) => O.Option<ReadonlyMap<K, A>>) => {
 ...
 * @since 3.0.0
 */
export const deleteAt = <K>(E: Eq<K>): ((k: K) => <A>(m: ReadonlyMap<K, A>) => O.Option<ReadonlyMap<K, A>>) => {
  const popE = pop(E)
 ...
 * @since 3.0.0
 */
export const pop = <K>(E: Eq<K>): ((k: K) => <A>(m: ReadonlyMap<K, A>) => O.Option<readonly [A, ReadonlyMap<K, A>]>) => {
  const lookupWithKeyE = lookupWithKey(E)
  return (k) => {
    const lookupWithKeyEk = lookupWithKeyE(k)
    return (m) => {
 ...
 * @since 3.0.0
 */
export const compact: Compactable2<URI>['compact'] = <K, A>(m: ReadonlyMap<K, O.Option<A>>): ReadonlyMap<K, A> => {
 ...
 * @since 3.0.0
 */
export const filterMapWithIndex = <K, A, B>(f: (k: K, a: A) => O.Option<B>) => (
  fa: ReadonlyMap<K, A>
): ReadonlyMap<K, B> => {
  const m = new Map<K, B>()
  const entries = fa.entries()
 ...
 * @since 3.0.0
 */
export const lookupWithKey = <K>(E: Eq<K>) => (k: K): (<A>(m: ReadonlyMap<K, A>) => O.Option<readonly [K, A]>) => {
 ...
 * @since 3.0.0
 */
export const lookup = <K>(E: Eq<K>): ((k: K) => <A>(m: ReadonlyMap<K, A>) => O.Option<A>) => {
  const lookupWithKeyE = lookupWithKey(E)
