 ...
 * @since 3.0.0
 */
export const insertAt = <A>(k: string, a: A) => (r: ReadonlyRecord<string, A>): O.Option<ReadonlyRecord<string, A>> => {
  if (!_.has.call(r, k)) {
 ...
 * @since 3.0.0
 */
export const updateAt = <A>(k: string, a: A): ((r: ReadonlyRecord<string, A>) => O.Option<ReadonlyRecord<string, A>>) =>
  modifyAt(k, () => a)
 ...
 * @since 3.0.0
 */
export const deleteAt = (k: string) => <A>(r: ReadonlyRecord<string, A>): O.Option<ReadonlyRecord<string, A>> => {
  if (!_.has.call(r, k)) {
    return _.none
  }
  const out: Record<string, A> = Object.assign({}, r)
 ...
 * @since 3.0.0
 */
export const pop = (k: string) => <A>(r: ReadonlyRecord<string, A>): O.Option<readonly [A, ReadonlyRecord<string, A>]> =>
  pipe(
    r,
    deleteAt(k),
    O.map((out) => [r[k], out])
...
export const compact: Compactable1<URI>['compact'] = <A>(
  r: ReadonlyRecord<string, O.Option<A>>
 ...
 * @since 3.0.0
 */
export const lookup = (k: string) => <A>(r: ReadonlyRecord<string, A>): O.Option<A> =>
